            ...
            if (isset($product['variations']) && isset($product['variations'][0]) && array_keys($product['variations'][0])) {
                ...
                foreach ($listAttributes as $attr) {
                    ...
                    if ($this->isProductAttributeExists($attrCode) == false && !in_array($attr, $attrListNotChecks)) {
                        ...
                        $attributeId = $eavSetup->getAttributeId(Product::ENTITY, 'size_modernrugs');
                        if ($attrCode == 'size_modernrugs') {
                            $arrOptionSize = [];
                            $arrValue = [];
                            foreach ($product['variations'] as $variation) {
                                if (isset($variation['size'])) {
                                    array_push($arrValue, $variation['size']);
                                }
                                $arrOptionSize['values'] = $arrValue;
                                $arrOptionSize['attribute_id'] = $attributeId;
                            }
                            $eavSetup->addAttributeOption($arrOptionSize);
                        }
                    }
                }
            }
...
//                $productAdd->setMediaGalleryEntries($mediaGallery);
                /** @var string $tmpDir */
                $tmpDir = $this->directoryList->getPath(DirectoryList::MEDIA) . DIRECTORY_SEPARATOR . 'tmp';
                /** create folder if it is not exists */
                $this->file->checkAndCreateFolder($tmpDir);
                /** @var string $newFileName */
                $newFileName = $tmpDir . baseName($product['image']);
                /** read file from URL and copy it to the new destination */
                $result = $this->file->read($product['image'], $newFileName);
                $imageType = [
                    "image",
                    "small_image",
                    "thumbnail",
                ...
                if ($result) {
                    ...
                    $productAdd->addImageToMediaGallery($newFileName, $imageType, true, false);
                }
//                $imagePath = $product['image']; // path of the image
//                $productAdd->addImageToMediaGallery($imagePath, array('image', 'small_image', 'thumbnail'), true, false);
...
//            $customerQuote = $this->quoteModel->loadByCustomerId($quote, 1); // where `$customerId` is your `customer id`
//            $items = $customerQuote->getData();
            $quote = $this->quoteFactory->create()->loadByCustomer(1);
            var_dump($quote->getId());

            //                /home/an/code/magento240/dev/tests/integration/testsuite/Magento/ConfigurableProduct/_files/quote_with_configurable_product.php
//                /home/an/code/magento240/vendor/magento/module-quote/Model/QuoteRepository/Plugin/AccessChangeQuoteControl.php
...
//                $quote->assignCustomer($customer);
                ...
                $productConfi = $this->productRepository->getById(2083);
...
//                            'attribute_id' => 'option_id',
                ...
                $cart->addProduct($productConfi, $requestInfo);
                $this->registry->register('customvar', 'annnnnnnnn');
                $cart->save();
